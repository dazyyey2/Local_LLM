...
def send_request(model='gemma3:27b', prompt='Describe this image.',
                 image_paths=None, stream=True, context=None, state={}):
    if isinstance(image_paths, str):
        image_paths = [image_paths]
    # If there has been an image given in earlier promt,
    # use that image for context.
    if image_paths is None and state.get('image_url'):
    ...
    if image_paths:  # If image is given.
        ...
        for path in image_paths:
            if not os.path.exists(path):
                ...
                if not os.path.exists(path):
                    # If given image is not found return 'NotFound'
                    return 'NotFound', state['image_url']
            try:
                images_b64.append(encode_image_to_base64(path))
            except Exception as e:
                print(f'Failed to read image {path}: {e}')
    else:  # If no image has been given
        ...
        for line in resp.iter_lines(decode_unicode=True):
            ...
            if data.get('done'):
                ...
                break
        # Return from not having been given an image.
        return final_context, state['image_url']
    # If image has been given:
    payload = {'model': model, 'prompt': prompt, 'stream': stream}
    if images_b64:
    ...
    for line in resp.iter_lines(decode_unicode=True):
        ...
        try:
            ...
            print('\nReceived non-JSON line from server.')
            continue
        text = data.get('response', '')
        if text:
            print(text, end='', flush=True)
        if data.get('done'):
            final_context = data.get('context')
            break
    # Return from being given an image
    return final_context, state['image_url']
...
def get_current_models():
    try:
        ...
        print(f'Could not fetch models from {OLLAMA_URL}api/tags\n{e}')
        return []
...
def print_menu(state):
    ...
    print('===============================================')
    user_input = str(
        input('Please enter choice of model (default: gemma3:27b): ')
    )
    if user_input == '':
...
def main():
    ...
    try:
        while True:  # Main program loop
            # If a model is selected, proceed with using that model.
            if state.get('current_model'):
                image_path = ''
                user_input = input('Enter a prompt (or /help): ').strip()
                if user_input == '/help':
                    print('/quit, /exit or /b - Exit to the menu.')
                    print('/img [imgURL] - Give the model image context')
                else:
                    if user_input.lower() not in {'/quit', '/exit', '/b'}:
                        counter = 0
                        # Find the image url after user writes "/img"
                        for word in user_input.split():
                            ...
                            if counter == 0:
                                if word == '/img':
                                    counter = 1
                    # If the user gives an image, process the image
                    if image_path:
                        print(f"Currently selected LLM: "
                              f"{state['current_model']}")
                        print('-----------------------------------------'
                              '-----------\n')
                        new_context, state['image_url'] = send_request(
                            ...
                            state=state
                        )  # Save the context in new_context.
                        # Update context.
                        if new_context is not None and \
                                new_context != 'NotFound':
                            state['context'] = new_context
                        # If image path is not found
                        if new_context == 'NotFound':
                            print('Image not found, please enter a valid '
                                  'image path.', end='')
                        print('\n\n-------------------------------------'
                              '---------------\n')
